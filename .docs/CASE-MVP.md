...
**录制用例仍进不了任务登记表**（登记表是代码，它是数据），但**已经能进常驻调度**：
调度项的 `taskId` 写成 `recorded:<会话 id>`，由设备侧增补层承载，
见 `RESIDENT.md` 的「设备侧增补层」。要让它变成仓库里的正式用例，
仍需拷回 `src/cases/`、把 `assetBase` 改回 `assets`、锚点移进 `src/assets/`，
再照下方「加一条新用例」登记。
